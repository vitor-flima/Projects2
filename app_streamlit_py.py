...
Original file is located at
    https://colab.research.google.com/drive/1kMMMBH9QgPrkQhB0eFBmxArtv136cU8C
"""
...
morador_1_rules = [
    ...
    ctrl.Rule(temperatura_do_ar['Hot'] & inicio_do_banho['Delayed'], duracao_do_banho['No shower']),
...
if temperaturas and duracao_simulacao > 0 and total_moradores_predio > 0:
    if st.sidebar.button("Executar Simulação"):
        ...
        for temperatura_atual, resultados in resultados_por_temperatura.items():
            ...
            # Add download button for results for this temperature
            df_results = pd.DataFrame({
                'Tempo (s)': resultados['tempo'],
                'Média Vazão (L/s)': resultados['media_ts'],
                'P5 Vazão (L/s)': resultados['p5_ts'],
                'P95 Vazão (L/s)': resultados['p95_ts']
            })
            ...
            st.download_button(
                label=f"Download Resultados ({temperatura_atual}°C)",
                data=csv_bytes,
                file_name=f"resultados_simulacao_vazao_temp_{temperatura_atual}C.csv",
                mime="text/csv"
            )
